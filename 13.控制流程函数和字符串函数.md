...
如果N_OR_S是一个数字，则返回一个十六进制值N的字符串表示，其中，`N`是一个`longlong`（也就是`BIGINT`）类型的数。如果N_OR_S是一个字符串，则返回值为一个N_OR_S的十六进制字符串表示，其中字符串N_OR_S 里的每个字符都被转化为两个十六进制数字。
...
返回`N`的八进制值的字符串表示，其中N是一个longlong(BIGINT)数。若N为NULL，则返回值为NULL。
```
mysql> SELECT OCT(12);
```
