...
在BEFORE触发程序中，`AUTO_INCREMENT`列的`NEW`值为0，不是实际插入新记录时将自动生成的序列号。
...
在下面的示例中，演示了这些要点。在该示例中，定义了1个UPDATE触发程序，用于检查更新每一行时将使用的新值，并更改值，使之位于0～100的范围内。它必须是BEFORE触发程序，这是因为需要在将值用于更新行之前对其进行检查：
```
mysql> delimiter //
